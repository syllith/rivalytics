...
export function renderMatchesCard({ username, season, currentRank, currentScore, combinedRows }) {
  ...
  const columns = [
    { key:'idx', label:'#', font: rowFontSpec }, { key:'map', label:'Map', font: rowFontSpec }, { key:'delta', label:'Rank Δ', font: rowFontSpec },
    { key:'rs', label:'RS After', font: rowFontSpec }, { key:'kd', label:'K/D', font: rowFontSpec }, { key:'dmg', label:'Damage', font: rowFontSpec },
    { key:'dur', label:'Dur', font: rowFontSpec }, { key:'time', label:'Time', font: smallFontSpec }, { key:'replay', label:'Replay', font: smallFontSpec }, { key:'rank', label:'Rank', font: smallFontSpec }
  ];
  ...
  let cursorX = tableX + 28; columns.forEach(c=>{ c.x = cursorX; cursorX += c.width + MIN_GAP; });
  const headerBaselineY = tableY + 54; columns.forEach(c => drawText(ctx, c.label, c.x, headerBaselineY, { font: headerFontSpec, color:'#AAB4CF' }));
  ...
  rows.forEach((_, i) => {
    const rowYBase = headerBaselineY + (i+1) * ROW_H;
    if (i % 2 === 0) { ctx.fillStyle = 'rgba(255,255,255,0.045)'; ctx.fillRect(tableX + 8, rowYBase - ROW_H + 10, tableW - 16, ROW_H - 8); }
    const data = rowStrings[i];
    columns.forEach(c => {
      if (c.key === 'kd') { // composite K/D textual layout
      ...
      let text = data[c.key] ?? '';
      if (c.key === 'map') { // truncate long map names
        let attempt = text; const f = colFont(c); mctx.font = f;
        while (mctx.measureText(attempt).width > c.width && attempt.length > 5) attempt = attempt.slice(0,-2) + '…';
        text = attempt;
      }
