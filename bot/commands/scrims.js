...
export async function handleScrimsCommand(message, args) {
  ...
  try {
    ...
    if (!data.data?.matches) return loadingMsg.edit('❌ No match data found for this user.');
    ...
    if (!matches.length) return loadingMsg.edit('❌ No recent matches found where modeName is "Unknown".');
    const embed = new EmbedBuilder().setTitle(`🎮 Scrim Matches for ${username}`).setColor(0x4B7BEC).setTimestamp();
    ...
    const avgDamage = matches.length? (totalDamage/matches.length): 0; const avgKD = totalDeaths>0? (totalKills/totalDeaths): totalKills;
